...
import type { FacultyInput, GroupMatchResult, StreamEvent } from '../types';
...
export const TeamBuilderPage: React.FC = () => {
    ...
    const [results, setResults] = useState<GroupMatchResult[]>([]);
        ...
        const abort = streamChat(
            {
                ...
                message: message.trim() || 'Find the best matching grants for this research team.',
            ...
            (event: StreamEvent) => {
                if (event.type === 'step_update') {
                    ...
                    if (event.payload.type === 'error') {
                        ...
                        setResults(event.payload.groupResults);
                    } else {
                        setInfoMessage(event.payload.message);
                    }
                }
                        ...
                        {results.length > 0 && (
                            <div className="space-y-4">
                                <h3 className="text-sm font-semibold text-slate-600">
                                    {results.length} Team Match{results.length > 1 ? 'es' : ''} Found
                                </h3>
                                {results.map((gm, idx) => {
